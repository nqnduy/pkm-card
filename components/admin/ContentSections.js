		...
		config.map((sectionCfg, index) => {
			...
			for (const [key, cfg] of Object.entries(sectionCfg)) {
				if (key != "section") {
					...
					switch (cfg.type) {
						case "text":
							input = <Input label={label} key={key + `-${inputValue}`} defaultValue={inputValue} onChange={onChange} />;
							break;
						case "textarea":
							input = <TextArea label={label} key={key + `-${inputValue}`} defaultValue={inputValue} onChange={onChange} />;
							break;
						case "select":
							input = (
