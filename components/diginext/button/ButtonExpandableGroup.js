...
const ButtonExpandableGroup = ({ label = "", icon, children, show = false, fontSize = "14px", href, color = "#6e84a3", colorActive = "black" }) => {
	...
	const btn = (
		<div className="btn" onClick={toggleDisplay}>
			...
			<HorizontalList type="start" align="middle">
				<span className="icon">
					{icon ? <AdminIcon name={icon} width="18px" height="auto" /> : <span style={{ width: "18px", display: "inline-block" }} />}
				</span>
				...
				{countChildren > 0 ? <AdminIcon name={isShow ? "arrow-expand" : "arrow-collapse"} width="8px" height="auto" /> : null}
			</HorizontalList>
		</div>
	);
