		...
		},
		{
			...
			render: function renderBadges(status, row, index) {
				return (
					<>
						{status == 1 ? (
							<AdminBadge style={{ marginBottom: "5px", marginRight: "5px" }} size={ButtonSize.SMALL} type={ButtonType.TRANSPARENT}>
								{row["statusText"]}
							</AdminBadge>
						) : (
							""
						...
						{status == 2 ? (
							<AdminBadge style={{ marginBottom: "5px", marginRight: "5px" }} size={ButtonSize.SMALL} type={ButtonType.LIGHT}>
								{row["statusText"]}
							</AdminBadge>
						) : (
							""
		...
		<LayoutPage header={header} user={user}>
			...
			<HorizontalList itemSize="stretch">
				<ListItem style={{ marginRight: "1rem" }}>
					<Input ref={(el) => (formSearchRef.current.fullNameNon = el)} label="Full Name" placeholder="Full Name" maxLength="255" />
				</ListItem>
				<ListItem style={{ marginRight: "1rem" }}>
