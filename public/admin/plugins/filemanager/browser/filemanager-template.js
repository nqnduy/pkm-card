(function () {
	...
	if ("classList" in HTMLElement.prototype && !("forEach" in document.createElement("_").classList)) {
	...
	function updateTheme(theme) {
		...
		if (themeOption) {
			...
			fmWrapper.querySelectorAll(".theme-option").forEach(function (item) {
	...
	// DOM Elements
	var editor = fmWrapper.querySelector("#fm-editor"),
	...
	on("click", ".files-table .file-item", function (e) {
		if (!e.target.closest(".checkbox")) {
			// Ignore event bubbling for the checkbox
			if (!e.shiftKey && !e.ctrlKey) {
				// Multiple selection with shift & ctrl keys
	...
	function doSelect(isSelectAll) {
		...
		[].forEach.call(allCheckboxes, function (checkbox) {
			...
			checkbox.closest(".file-item").classList[isSelectAll ? "add" : "remove"]("selected");
		});
	}
	...
	function selectByIndex(index) {
		var item = fmWrapper.querySelector(".files-table .file-item:nth-child(" + index + ")"),
	...
	var moveFileModal = fmWrapper.querySelector("#moveFileModal");

	// Select file item event
	on("click", "#moveFileModal .files-list .item", function (e) {
		var ele = e.target.closest(".item");
		[selectFileItem, updateDestinationFolder].forEach(
			function (func) {
				func.call(this, e);
			}.bind(ele)
		);
	...
	fmWrapper.querySelector('*[data-action="move"]').addEventListener("click", function () {
	...
	function updateSourceFileName() {
		var tableSelectedFileName = fmWrapper.querySelector(".files-table .file-item.selected .file-name"),
	...
	fmWrapper.querySelector('*[data-action="remove"]').addEventListener("click", function () {
		var tableSelectedItems = fmWrapper.querySelectorAll(".files-table .file-item.selected"),
	...
	fmWrapper.querySelector('*[data-action="rename"]').addEventListener("click", function () {
		var tableSelectedItem = fmWrapper.querySelector(".files-table .file-item.selected"),
